...
class TrajectoryDataset(Dataset):
    def __init__(self, dataset=None, trs=None, view_range=None, window=3, config=None, aug=None):
        self.data_root = "/home/sdas24/scripts/ftpv_dataset"
        self.config = config
    ...
    def load_data(self, tr):
        ...
        for view in self.view_range:
            for i in range(n_frames):
                ...
                x_ts.append(t_tn)
                if i == 0:
                    p_ts.append(torch.ones(1, 224, 224))
                    last_frame = t
                else:
                    new_frame_pos = ImageChops.subtract(last_frame, t)
                    gray_image3 = new_frame_pos.convert('L')

                    # create binary image
                    threshold_value = 127
                    binary_image_diff = gray_image3.point(lambda x: 0 if x < threshold_value else 255, '1')
                    p_ts.append(to_tensor(binary_image_diff))
                    last_frame = t
        ...
        view_size = x_f.size(0)
        p_f = torch.ones((n_frames, 1, 224, 224))
        p_t = torch.stack([x for x in p_ts]).reshape(len(self.view_range), n_frames, 1, 224, 224)
        x = torch.cat((x_f.unsqueeze(0), x_t))
        p = torch.cat((p_f.unsqueeze(0), p_t))
        # print(x.size(), p.size())
        return x, p # V, T, C, H, W, on CPU
...
class TripletDataset(Dataset):
    def __init__(self, data=None, pose_map=None, window=3, n_pos=1, n_neg=1, device='cpu'):
    ...
    def __getitem__(self, idx):
        ...
        # print(self.data.size(), self.pose_map.size())
        ...
        return self.data[i][indices], self.data[j][indices], self.pose_map[i][indices], self.pose_map[j][indices]

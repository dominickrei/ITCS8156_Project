...
parser.add_argument('--port', type=str, default="64754")
...
def load_data(tr, sample_indices, view_range):
    base_path = f"/home/dreilly1/Projects/3DTRL/ftpv_dataset/{dataset}/{tr}"
...
if not args.test:
    ...
    for ep in range(args.num_episodes):
        ...
        eval_taus = []
        print('\tdoing eval')
        for i, tr in enumerate(eval_trs):
            if i%10==0:
                print(f'\t-iter {i}/{len(eval_trs)}')

            path = f"/home/dreilly1/Projects/3DTRL/ftpv_dataset/{dataset}/{tr}/fpv"
...
test_taus = []
print('doing test')
for i, tr in enumerate(test_trs):
    if i%10==0:
        print(f'\t-iter {i}/{len(test_trs)}')
